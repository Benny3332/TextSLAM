...
namespace TextSLAM
{
     ...
     * frame CurrentFrame
     */
    frame::frame(const cv::Mat& imGray, const double& ImgTimeStamp, const Mat33& K, int& ScaleLevels,
                 double& ScaleFactor, const vector<vector<Vec2>>& TextDete, const vector<TextInfo>& TextMean,
                 ORBextractor* extractor, const bool& bVeloc):
        dTimeStamp(ImgTimeStamp), iScaleLevels(ScaleLevels), mK(K), dScaleFactor(ScaleFactor), vTextDete(TextDete),
        vTextMean(TextMean), coORBextractor(extractor), bVelocity(bVeloc)
    {
        // 1. basic param
    ...
    void frame::GetFrameParam()
    {
        ...
        if (DEBUG)
        {
            cout << "fx, fy, cx, cy, invfx, invfy are: " << fx << ", " << fy << ", " << cx << ", " << cy << ", " <<
                invfx << ", " << invfy << endl;
    ...
    void frame::FeatExtract()
    {
        ...
        bool FLAG_NOSCENEMASK = true;
        // use for TextSLAM comparised with pts. If true(compair), extract scene features within the text features extraction
        // ---- log ----
    ...
    */
    void frame::FeatFusion()
    {
        ...
        if (iN != (IdxText + IdxBeginText) || iN != mDescr.rows || iNScene != mDescrScene.rows || iNTextObj !=
            mDescrText.size())
        {
    ...
    void frame::FeatExtracText(const cv::Mat& Img, const vector<vector<Vec2>>& TextDete,
                               vector<vector<cv::KeyPoint>>& KeysTextRaw, vector<cv::Mat>& DescripTextRaw)
    {
    ...
    void frame::TextFeaProc()
    {
        for (size_t i0 = 0; i0 < iNTextObj; i0++)
        {
            ...
            Tool.GetPyramidPts(vKeysTextObj, vTextDeteMin[i0], vTextDeteMax[i0], vFrameImg, vFrameGrad,
                               vInvScaleFactors, vK_scale, vTextfeature);
    ...
    void frame::AddSceneObserv(mapPts* scenept, const int& ScenePtId, int idx)
    {
        ...
        for (size_t ipyr = 0; ipyr < vSceneObv2d.size(); ipyr++)
        {
            ...
            SceneFeature* featureInPyr = new SceneFeature{
                Observ2DPyr(0), Observ2DPyr(1), Observ2DPyr, (int)ipyr, Idx2Raw
            };
